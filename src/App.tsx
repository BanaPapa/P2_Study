...
function TreeNode(props: {
  ...
  const totalEntries = countEntries(node);
          ...
          <div className={`em-picker ${sheet === "emoji" ? "show" : ""}`}>
            ...
            <div className="em-tabs">
              {emojiCategories.map((c, i) => (
                <button
                  key={c.id}
                  className={`em-tab ${i === emojiTab ? "active" : ""}`}
                  onClick={() => setEmojiTab(i)}
                >{c.icon}</button>
              ))}
            </div>
            <div className="em-grid" key={emojiCategories[emojiTab].id}>
              {emojiCategories[emojiTab].list.map((emoji, i) => (
                <button key={`${emoji}-${i}`} onClick={() => {
      ...
      <div className={`modal category-modal ${leaf ? "category-modal-leaf" : ""}`}>
        ...
        {emojiOpen && (
          <>
            ...
            <div className="mini-emoji-tabs">
              {emojiCategories.map((c, i) => (
                <button
                  key={c.id}
                  type="button"
                  className={`mini-emoji-tab ${i === emojiTab ? "active" : ""}`}
                  title={c.label}
                  onClick={() => setEmojiTab(i)}
                >{c.icon}</button>
              ))}
            </div>
            <div className="mini-emoji-grid" key={emojiCategories[emojiTab].id}>
              {emojiCategories[emojiTab].list.map((item, i) => (
                <button
    ...
    <div className="modal-overlay show" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="emoji-picker show">
        ...
        <div className="ep-grid" key={cat.id}>
          {cat.list.map((emoji, i) => <button key={`${emoji}-${i}`} onClick={() => onPick(emoji)}>{emoji}</button>)}
        </div>
      </div>
    </div>
  );
